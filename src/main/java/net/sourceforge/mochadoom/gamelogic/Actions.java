...
import static net.sourceforge.mochadoom.data.Defines.MAPBLOCKSIZE;
import static net.sourceforge.mochadoom.data.Defines.MAPBTOFRAC;
...
import static net.sourceforge.mochadoom.data.Defines.MTF_AMBUSH;
...
import static net.sourceforge.mochadoom.data.Defines.PT_ADDLINES;
import static net.sourceforge.mochadoom.data.Defines.PT_ADDTHINGS;
import static net.sourceforge.mochadoom.data.Defines.PT_EARLYOUT;
...
import static net.sourceforge.mochadoom.doom.English.PD_BLUEO;
import static net.sourceforge.mochadoom.doom.English.PD_REDK;
...
import static net.sourceforge.mochadoom.gamelogic.ChaseDirections.DI_SOUTH;
import static net.sourceforge.mochadoom.gamelogic.ChaseDirections.DI_SOUTHEAST;
import static net.sourceforge.mochadoom.gamelogic.ChaseDirections.DI_WEST;
import static net.sourceforge.mochadoom.gamelogic.ChaseDirections.diags;
import static net.sourceforge.mochadoom.gamelogic.ChaseDirections.opposite;
import static net.sourceforge.mochadoom.gamelogic.ChaseDirections.xspeed;
import static net.sourceforge.mochadoom.gamelogic.ChaseDirections.yspeed;
import static net.sourceforge.mochadoom.gamelogic.DoorDefines.FASTDARK;
import static net.sourceforge.mochadoom.gamelogic.DoorDefines.SLOWDARK;
import static net.sourceforge.mochadoom.gamelogic.DoorDefines.VDOORSPEED;
import static net.sourceforge.mochadoom.gamelogic.DoorDefines.VDOORWAIT;
import static net.sourceforge.mochadoom.gamelogic.MapUtils.AproxDistance;
import static net.sourceforge.mochadoom.gamelogic.mobj_t.MF_AMBUSH;
import static net.sourceforge.mochadoom.gamelogic.mobj_t.MF_CORPSE;
...
import static net.sourceforge.mochadoom.gamelogic.mobj_t.MF_DROPPED;
import static net.sourceforge.mochadoom.gamelogic.mobj_t.MF_FLOAT;
import static net.sourceforge.mochadoom.gamelogic.mobj_t.MF_INFLOAT;
...
import static net.sourceforge.mochadoom.gamelogic.mobj_t.MF_SKULLFLY;
...
/**
 * Action functions need to be aware of: Random number generator (RND) Broader gameplay context
 * (DoomGame) Some door actions Playfield checks. etc. etc. so they COULD be made into a separate
 * file
 *
 * @author admin
 */
...
public class Actions extends UnifiedGameMap {
  ...
  public void ClearCeilingsBeforeLoading() {
    ...
    /*
     * for (int i = 0; i < MAXPLATS; i++) { this.activeceilings[i] = null; }
     */
  }
  ...
  protected void ActivateInStasisCeiling(line_t line) {
    ...
    for (i = 0; i < getMaxCeilings(); i++) {
      if (getActiveCeilings()[i] != null && (getActiveCeilings()[i].tag == line.tag)
          && (getActiveCeilings()[i].direction == 0)) {
        getActiveCeilings()[i].direction = getActiveCeilings()[i].olddirection;
        getActiveCeilings()[i].function = think_t.T_MoveCeiling;
  ...
  protected void MoveFloor(floormove_t floor) {
    ...
    res = MovePlane(floor.sector, floor.speed, floor.floordestheight, floor.crush, 0,
        floor.direction);
   ...
   * <p/>
   * (I'm sure it has something to do with John Romero's obsession with fucking stuff and making
   * them his bitches).
   *
   * @param line
   */
    ...
    while ((secnum = FindSectorFromLineTag(line, secnum)) >= 0) {
      ...
      switch (floortype) {
        ...
        case raiseFloor:
          ...
          floor.floordestheight -= (8 * FRACUNIT) * eval(floortype == floor_e.raiseFloorCrush);
          break;
          ...
          for (i = 0; i < sec.linecount; i++) {
            if (twoSided(secnum, i)) {
              ...
              if (side.bottomtexture >= 0)
                if (TM.getTextureheight(side.bottomtexture) < minsize)
                  minsize = TM.getTextureheight(side.bottomtexture);
  ...
  protected void VerticalDoor(vldoor_t door) {
    ...
    switch (door.direction) {
      ...
      case -1:
        // DOWN
        res = MovePlane(door.sector, door.speed, door.sector.floorheight, false, 1, door.direction);
        if (res == result_e.pastdest) {
          switch (door.type) {
            case blazeRaise:
            case blazeClose:
  ...
  protected boolean DoLockedDoor(line_t line, vldoor_e type, mobj_t thing) {
    player_t p;
    ...
    switch (line.special) {
      ...
      case 137:
         ...
         * if ( p==null ) return false;
         */
        if (!p.cards[Card.it_yellowcard.ordinal()] && !p.cards[Card.it_yellowskull.ordinal()]) {
    ...
    switch (line.special) {
      ...
      case 32:
        ...
        if (!player.cards[Card.it_bluecard.ordinal()]
            && !player.cards[Card.it_blueskull.ordinal()]) {
  ...
  /**
   * Tries to move a mobj_t in its current direction. For actors of the monster_t type, applies
  ...
  void NewChaseDir(mobj_t actor) {
    ...
    if (RND.P_Random() > 200 || Math.abs(deltay) > Math.abs(deltax)) {
      tdir = d1;
      d1 = d2;
      d2 = tdir;
    }
  ...
  /**
   * TryWalk Attempts to move actor on in its current (ob.moveangle) direction. If blocked by either
   * a wall or an actor returns FALSE If move is either clear or blocked only by a door, returns
   * TRUE and sets... If a door is in the way, an OpenDoor call is made to start it opening.
   */
  ...
  /**
   * P_SpawnPlayer Called when a player is spawned on the level. Most of the player structure stays
  ...
  mobj_t SpawnMapThing(mapthing_t mthing) {
    ...
    if (i == NUMMOBJTYPES) {
      System.err.printf("P_SpawnMapThing: Unknown type %d at (%d, %d)", mthing.type, mthing.x,
          mthing.y);
      return null;
    }
  ...
  public void DamageMobj(mobj_t target, mobj_t inflictor, mobj_t source, int damage) {
    ...
    if ((inflictor != null) && !eval(target.flags & MF_NOCLIP) && (source == null
        || source.player == null || source.player.readyweapon != weapontype_t.wp_chainsaw)) {
      ang = R.PointToAngle2(inflictor.x, inflictor.y, target.x, target.y) & BITS32;
    ...
    if (player != null) {
      ...
      if (damage < 1000 && (eval(player.cheats & player_t.CF_GODMODE))
    ...
    if ((RND.P_Random() < target.info.painchance) && !eval(target.flags & MF_SKULLFLY)) {
    ...
    if (((target.threshold == 0) || (target.type == mobjtype_t.MT_VILE)) && (source != null)
        && (source != target) && (source.type != mobjtype_t.MT_VILE)) {
  ...
  private mobj_t SpawnSkull(mobj_t actor, long angle) {
    ...
    prestep =
        4 * FRACUNIT + 3 * (actor.info.radius + mobjinfo[mobjtype_t.MT_SKULL.ordinal()].radius) / 2;
  ...
  public void KillMobj(mobj_t source, mobj_t target) {
    ...
    if (target.player != null) {
      ...
      if (target.player == DM.players[DM.consoleplayer] && DM.automapactive) {
  ...
  /**
   * P_CrossSpecialLine - TRIGGER Called every time a thing origin is about to cross a line with a
   * non 0 special.
   */
  ...
  public class PTR_ShootTraverse implements PTR_InterceptFunc {
    ...
    // _D_: NOTE: this function was added, because replacing a goto by a boolean flag caused a bug
    // if shooting a single sided line
  ...
  // P_TeleportMove
  //
  public boolean TeleportMove(mobj_t thing, int x, // fixed
      int y) {
    int xl;
    int xh;
    int yl;
   ...
   * <p/>
   * in: a mobj_t (can be valid or invalid) a position to be checked (doesn't need to be related to
   * the mobj_t.x,y)
   * <p/>
   * during: special things are touched if MF_PICKUP early out on solid lines?
   * <p/>
   * out: newsubsec floorz ceilingz tmdropoffz the lowest point contacted (monsters won't move to a
  ...
  /**
   * // P_TryMove // Attempt to move to a new position, // crossing special lines unless MF_TELEPORT
   * is set.
   *
   * @param x fixed_t
   * @param y fixed_t
  ...
  boolean TryMove(mobj_t thing, int x, int y) {
    ...
    if (!eval(thing.flags & MF_NOCLIP)) {
      ...
      if (!eval(thing.flags & MF_TELEPORT) && tmceilingz - thing.z < thing.height)
        return false; // mobj must lower itself to fit

      if (!eval(thing.flags & MF_TELEPORT) && tmfloorz - thing.z > 24 * FRACUNIT)
        return false; // too big a step up

      if (!eval(thing.flags & (MF_DROPOFF | MF_FLOAT)) && tmfloorz - tmdropoffz > 24 * FRACUNIT)
    ...
    do {
      ...
      if (!TryMove(mo, ptryx, ptryy)) {
        ...
        if (mo.player != null) { // try to slide along it
          ...
          // explode a missile
          if (ceilingline != null && ceilingline.backsector != null
  ...
  /**
   * P_UseSpecialLine Called when a thing uses a special line. Only the front sides of lines are
   * usable.
   */
  ...
  /**
   * P_BlockLinesIterator The validcount flags are used to avoid checking lines that are marked in
   * multiple mapblocks, so increment validcount before the first call to P_BlockLinesIterator, then
  ...
  /**
   * P_PathTraverse Traces a line from x1,y1 to x2,y2, calling the traverser function for each.
   ...
   * @param direction
   */
  result_e MovePlane(sector_t sector, int speed, int dest, boolean crush, int floorOrCeiling,
      int direction) {
    boolean flag;
    int lastpos; // fixed_t
  ...
  public void Ticker() {
    ...
    if (!DM.netgame && DM.menuactive && !DM.demoplayback
  ...
  void SpawnSpecials() {
     ...
     * int episode;
     * 
     * episode = 1; if (W.CheckNumForName("texture2") >= 0) episode = 2;
     */
  ...
  public void RunThinkers() {
    ...
    while (currentthinker != thinkercap) {
      if (currentthinker.function == think_t.NOP) {
         ...
         * try {
         * 
         * // According to certian gurus, this method is faster than instanceof // and almost on par
         * with id checking. // //
         * http://stackoverflow.com/questions/103564/the-performance-impact-of-using-instanceof-in-
         * java if (currentthinker.getClass()==mobj_t.class)
         * 
         * mobjpool.checkIn((mobj_t)currentthinker); } catch (ClassCastException e){ // Object will
         * simply be destroyed without reuse, in this case. }
         */
  ...
  class PIT_VileCheck implements PIT_MobjFunction {
    ...
    public boolean invoke(mobj_t thing) {
      ...
      if (Math.abs(thing.x - viletryx) > maxdist || Math.abs(thing.y - viletryy) > maxdist)
  ...
  private class PIT_ChangeSector implements PIT_MobjFunction {
    public boolean invoke(mobj_t thing) {
      ...
      if (crushchange && !eval(DM.leveltime & 3)) {
        ...
        mo = A.SpawnMobj(thing.x, thing.y, thing.z + thing.height / 2, mobjtype_t.MT_BLOOD);
      ...
      if (eval(tmthing.flags & MF_MISSILE)) {
        ...
        if (tmthing.target != null
            && (tmthing.target.type == thing.type
                || (tmthing.target.type == mobjtype_t.MT_KNIGHT
                    && thing.type == mobjtype_t.MT_BRUISER)
                || (tmthing.target.type == mobjtype_t.MT_BRUISER
