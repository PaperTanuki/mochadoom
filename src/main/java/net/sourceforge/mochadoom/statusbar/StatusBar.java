...
import static net.sourceforge.mochadoom.automap.IAutoMap.AM_MSGENTERED;
import static net.sourceforge.mochadoom.automap.IAutoMap.AM_MSGEXITED;
import static net.sourceforge.mochadoom.automap.IAutoMap.AM_MSGHEADER;
import static net.sourceforge.mochadoom.data.Defines.NUMAMMO;
import static net.sourceforge.mochadoom.data.Defines.NUMCARDS;
...
import static net.sourceforge.mochadoom.data.Defines.pw_ironfeet;
import static net.sourceforge.mochadoom.data.Defines.pw_strength;
...
import static net.sourceforge.mochadoom.doom.English.STSTR_BEHOLD;
import static net.sourceforge.mochadoom.doom.English.STSTR_BEHOLDX;
import static net.sourceforge.mochadoom.doom.English.STSTR_CHOPPERS;
import static net.sourceforge.mochadoom.doom.English.STSTR_CLEV;
import static net.sourceforge.mochadoom.doom.English.STSTR_DQDOFF;
import static net.sourceforge.mochadoom.doom.English.STSTR_DQDON;
import static net.sourceforge.mochadoom.doom.English.STSTR_FAADDED;
import static net.sourceforge.mochadoom.doom.English.STSTR_KFAADDED;
import static net.sourceforge.mochadoom.doom.English.STSTR_MUS;
import static net.sourceforge.mochadoom.doom.English.STSTR_NCOFF;
import static net.sourceforge.mochadoom.doom.English.STSTR_NCON;
...
import static net.sourceforge.mochadoom.video.DoomVideoRenderer.V_PREDIVIDE;
import static net.sourceforge.mochadoom.video.DoomVideoRenderer.V_SCALEOFFSET;
import static net.sourceforge.mochadoom.video.DoomVideoRenderer.V_TRANSLUCENTPATCH;
...
public class StatusBar extends AbstractStatusBar {
  ...
  /**
   * whether left-side main status bar is active. This fugly hax (and others like it) are necessary
   * in order to have something close to a pointer.
   */
  ...
  // Yeah, right...
  private char cheat_mus_seq[] = {0xb2, 0x26, 0xb6, 0xae, 0xea, 1, 0, 0, 0xff};
  ...
  //
  private char cheat_commercial_noclip_seq[] = {0xb2, 0x26, 0xe2, 0x36, 0xb2, 0x2a, 0xff // idclip
  };
  ...
  private char cheat_clev_seq[] = {0xb2, 0x26, 0xe2, 0x36, 0xa6, 0x6e, 1, 0, 0, 0xff // idclev
  };

  // my position cheat
  private char cheat_mypos_seq[] = {0xb2, 0x26, 0xb6, 0xba, 0x2a, 0xf6, 0xea, 0xff // idmypos
  };

  private char cheat_vampire_seq[] = {0xb2, 0x26, 0x6e, 0xa2, 0xb6, 0x2a, 0xff // idvamp
  };
  ...
  cheatseq_t[] cheat_powerup = {new cheatseq_t(cheat_powerup_seq[0], 0),
      new cheatseq_t(cheat_powerup_seq[1], 0), new cheatseq_t(cheat_powerup_seq[2], 0),
      new cheatseq_t(cheat_powerup_seq[3], 0), new cheatseq_t(cheat_powerup_seq[4], 0),
      new cheatseq_t(cheat_powerup_seq[5], 0), new cheatseq_t(cheat_powerup_seq[6], 0)};
  ...
  public boolean Responder(event_t ev) {
    ...
    // if a user keypress...
      ...
      if (!DM.netgame) {
        ...
        // Cheat for vampires
        else if (cheat_vamp.CheckCheat((char) ev.data1)) {
          plyr.message = STSTR_VAMP;
        }
        // Cheat for werewolfs
        ...
        // 'mypos' for player position
          ...
          // MAES: made into a toggleable cheat.
          plyr.message =
              (DM.flashing_hom = !DM.flashing_hom) ? "HOM Detection On" : "HOM Detection Off";
        }
      }
      ...
      if (cheat_clev.CheckCheat((char) ev.data1)) {
        ...
        if (DM.isRetail() && ((epsd > 4) || (map > 9)))
          return false;

        // MAES: If it's doom.wad but not ultimate
        if (DM.isRegistered() && !DM.isRetail() && ((epsd > 3) || (map > 9)))
          return false;

        if (DM.isShareware() && ((epsd > 1) || (map > 9)))
          return false;

        if (DM.isCommercial() && ((epsd > 1) || (map > 34)))
          return false;
  ...
  /**
   * This is a not-very-pretty routine which handles the face states and their timing. the
   * precedence of expressions is: dead > evil grin > turned head > straight ahead
   */
  public void updateFaceWidget() {
    ...
    if (priority < 8) {
      if ((plyr.damagecount != 0) && (plyr.attacker != null) && (plyr.attacker != plyr.mo)) {
        ...
        if (plyr.health[0] - st_oldhealth > ST_MUCHPAIN) {
          ...
          badguyangle = R.PointToAngle2(plyr.mo.x, plyr.mo.y, plyr.attacker.x, plyr.attacker.y);
    ...
    if (priority < 5) {
      // invulnerability
      if (((plyr.cheats & CF_GODMODE) != 0) || (plyr.powers[pw_invulnerability] != 0)) {
        priority = 4;
  ...
  /**
   * MAES: this code updated the widgets. Now, due to the way they are constructed, they originally
   * were "hooked" to actual variables using pointers so that they could tap into them directly and
   * self-update. Clearly we can't do that in Java unless said variables are inside an array and we
   * provide both the array AND an index. For other cases, we must simply build ad-hoc hacks.
   * <p/>
   * In any case, only "status" updates are performed here. Actual visual updates are performed by
   * the Drawer.
   */
  ...
  public void updateWidgets() {
    ...
    if (this.st_idmypos) {
      ...
      plyr.message = String.format("ang= 0x%x; x,y= (%x, %x)", (int) mo.angle, mo.x, mo.y);
  ...
  public void doPaletteStuff() {
    ...
    } else if (plyr.powers[pw_ironfeet] > 4 * 32 || (plyr.powers[pw_ironfeet] & 8) != 0)
  ...
  /**
   * Widgets are created here. Be careful, because their "constructors" used reference to boolean or
   * int variables so that they could be auto-updated by the global refresh functions. We can only
   * do this with some limitations in Java (e.g. passing an array AND an index).
   */
  ...
  public void createWidgets() {
    ...
    w_ready = new st_number_t(ST_AMMOX, ST_AMMOY, tallnum, plyr.ammo,
        weaponinfo[plyr.readyweapon.ordinal()].ammo.ordinal(), st_statusbaron, 0, ST_AMMOWIDTH);

    // the last weapon type
    w_ready.data = plyr.readyweapon.ordinal();

    // health percentage
    w_health = new st_percent_t(ST_HEALTHX, ST_HEALTHY, tallnum, plyr.health, 0, st_statusbaron, 0,
        tallpercent);

    // arms background
    w_armsbg =
        new st_binicon_t(ST_ARMSBGX, ST_ARMSBGY, armsbg, st_notdeathmatch, 0, st_statusbaron, 0);

    // weapons owned
    for (i = 0; i < 6; i++) {
      w_arms[i] = new st_multicon_t(ST_ARMSX + (i % 3) * ST_ARMSXSPACE,
          ST_ARMSY + (i / 3) * ST_ARMSYSPACE, arms[i], plyr.weaponowned, i + 1, st_armson, 0);
    }

    // frags sum
    w_frags = new st_number_t(ST_FRAGSX, ST_FRAGSY, tallnum, st_fragscount, 0, // dummy,
        // we're
        // passing
        // an
        // integer.
    ...
    // faces
    w_faces = new st_multicon_t(ST_FACESX, ST_FACESY, faces, st_faceindex, 0, st_statusbaron, 0);

    // armor percentage - should be colored later
    w_armor = new st_percent_t(ST_ARMORX, ST_ARMORY, tallnum, plyr.armorpoints, 0, st_statusbaron,
        0, tallpercent);

    // keyboxes 0-2
    w_keyboxes[0] = new st_multicon_t(ST_KEY0X, ST_KEY0Y, keys, keyboxes, 0, st_statusbaron, 0);

    w_keyboxes[1] = new st_multicon_t(ST_KEY1X, ST_KEY1Y, keys, keyboxes, 1, st_statusbaron, 0);

    w_keyboxes[2] = new st_multicon_t(ST_KEY2X, ST_KEY2Y, keys, keyboxes, 2, st_statusbaron, 0);

    // ammo count (all four kinds)

    w_ammo[0] = new st_number_t(ST_AMMO0X, ST_AMMO0Y, shortnum, plyr.ammo, 0, st_statusbaron, 0,
        ST_AMMO0WIDTH);

    w_ammo[1] = new st_number_t(ST_AMMO1X, ST_AMMO1Y, shortnum, plyr.ammo, 1, st_statusbaron, 0,
        ST_AMMO1WIDTH);

    w_ammo[2] = new st_number_t(ST_AMMO2X, ST_AMMO2Y, shortnum, plyr.ammo, 2, st_statusbaron, 0,
        ST_AMMO2WIDTH);

    w_ammo[3] = new st_number_t(ST_AMMO3X, ST_AMMO3Y, shortnum, plyr.ammo, 3, st_statusbaron, 0,
        ST_AMMO3WIDTH);

    // max ammo count (all four kinds)
    w_maxammo[0] = new st_number_t(ST_MAXAMMO0X, ST_MAXAMMO0Y, shortnum, plyr.maxammo, 0,
        st_statusbaron, 0, ST_MAXAMMO0WIDTH);

    w_maxammo[1] = new st_number_t(ST_MAXAMMO1X, ST_MAXAMMO1Y, shortnum, plyr.maxammo, 1,
        st_statusbaron, 0, ST_MAXAMMO1WIDTH);

    w_maxammo[2] = new st_number_t(ST_MAXAMMO2X, ST_MAXAMMO2Y, shortnum, plyr.maxammo, 2,
        st_statusbaron, 0, ST_MAXAMMO2WIDTH);

    w_maxammo[3] = new st_number_t(ST_MAXAMMO3X, ST_MAXAMMO3Y, shortnum, plyr.maxammo, 3,
        st_statusbaron, 0, ST_MAXAMMO3WIDTH);

  }

  /**
   * Binary Icon widget This is used for stuff such as keys or weapons, which you either have or you
   * don't.
   */

  class st_binicon_t implements StatusBarWidget {
    ...
    public void update(boolean refresh) {
      ...
      if (bi.on[onindex] && ((bi.oldval != bi.val[valindex]) || refresh)) {
        ...
        if (bi.val[valindex])
          ...
          V.CopyRect(x / vs.getScalingX(), y / vs.getScalingY() - ST_Y, BG, w * BEST_X_SCALE,
              h * BEST_Y_SCALE, x, y, FG);
  ...
  class st_multicon_t implements StatusBarWidget {
    ...
    public st_multicon_t(int x, int y, patch_t[] il, Object iarray, int inum, boolean[] on,
        int onindex) {
      this.x = x;
      this.y = y;
      this.oldinum = -1;
    ...
    public void update(boolean refresh) {
      ...
      if (this.on[onindex] && ((this.oldinum != thevalue) || refresh) && (thevalue != -1)) {
        ...
        // System.out.printf("Drawing at x y %d %d w h %d
        // %d\n",this.x,this.y,p[thevalue].width,p[thevalue].height);
        V.DrawScaledPatch(this.x, this.y, V_SCALEOFFSET | V_NOSCALESTART | FG, vs,
            this.p[thevalue]);
  ...
  class st_number_t implements StatusBarWidget {
     ...
     * <p/>
     * Fun fact: initially I tried to use Integer and Boolean, but those are immutable -_-. Fuck
     * that, Java.
     */
    int[] numarray;
    ...
    public st_number_t(int x, int y, patch_t[] pl, int[] numarray, int numindex, boolean[] on,
        int onindex, int width) {
      init(x, y, pl, numarray, numindex, on, onindex, width);
    }

    public void init(int x, int y, patch_t[] pl, int[] numarray, int numindex, boolean[] on,
        int onindex, int width) {
      this.x = x;
      this.y = y;
      this.oldnum = 0;
    ...
    public void drawNum(boolean refresh) {
      ...
      if (neg)
        V.DrawScaledPatch/* DrawPatch */(x - 8 * BEST_X_SCALE, this.y,
            FG | V_NOSCALESTART | V_TRANSLUCENTPATCH, vs, sttminus);
  ...
  class st_percent_t implements StatusBarWidget {
    ...
    public st_percent_t(int x, int y, patch_t[] pl, int[] numarray, int numindex, boolean[] on,
        int onindex, patch_t percent) {
